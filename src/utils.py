...
def add_documentation_chunks_to_supabase(
    client: Client,
    ...
    """
    Add documentation chunks to Supabase with proper content_type and heading_hierarchy.
    
    This function is specifically designed for user manual documentation chunks
    generated by the UserManualChunker pipeline.
    ...
    Args:
        ...
        chunks: List of chunk dictionaries with keys:
            ...
            - metadata: ChunkMetadata dict with:
                - source_file: Source file path
                - heading_hierarchy: List of heading strings
                - section_level: Depth in document hierarchy
                - contains_code: Boolean flag
                - code_languages: List of language strings
    ...
    print(f"Adding {len(chunks)} documentation chunks to Supabase...")
    
    # Extract unique source files for deletion
    if delete_existing:
        unique_sources = list(set(chunk['metadata']['source_file'] for chunk in chunks))
        print(f"Deleting existing records for {len(unique_sources)} source files...")
        
        try:
            # Delete by source_file in metadata
    ...
    for i in range(0, len(chunks), batch_size):
        ...
        for chunk in batch_chunks:
            ...
            record = {
                "url": source_file,  # Use source file path as URL
                ...
                if retry < max_retries - 1:
                    ...
                    for record in batch_data:
                        try:
                            ...
                            print(f"Failed to insert record {record.get('url', 'unknown')}: {individual_error}")
                            failed_inserts += 1
    
    print(f"\nDocumentation chunks insertion complete:")
